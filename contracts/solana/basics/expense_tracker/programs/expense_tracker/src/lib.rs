...
declare_id!("EQQoQt2ta7eePCQ2U9CmfKMDtc84vqeZurtfy4Q7uEaB");
...
pub mod expense_tracker {
    ...
    pub fn initialize_expense(
        ...
        expense_account.name = merchant_name;
        expense_account.amount = amount;
        expense_account.owner = *ctx.accounts.auth.key;
...
pub struct InitializeExpense<'info> {
    ...
    #[account(
        ...
        payer = auth,
        space = 1000,
        seeds = [b"expense", auth.key().as_ref(), id.to_le_bytes().as_ref()],
        bump
    )]
